...
function getEmbeddableImageSrc(url) {
    if (!url || typeof url !== "string") return "";

    // 1. Google Drive file link (drive.google.com)
    ...
    // 2. Google Usercontent link with id= parameter
    const usercontentRegex = /https?:\/\/drive\.usercontent\.google\.com\/download\?id=([a-zA-Z0-9_-]+)&?export=view?/;
    const ucMatch = url.match(usercontentRegex);
    if (ucMatch) {
        const fileId = ucMatch[1];
        return `https://drive.google.com/uc?export=view&id=${fileId}`;
    }

    // 3. Regular URL — return as is
    return url;
}
